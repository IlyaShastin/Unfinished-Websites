...
<?php
    class User {
        ...
        public function __construct( $st ) {
            ...
            $cached = self::isCached( $st );
            if ( isset($cached) ) {
                $this->profileLink      = $cached->profileLink;
                $this->lastOnline       = $cached->lastOnline;
                $this->name             = $cached->name;
                $this->avatar           = $cached->avatar;
                $this->avatarMedium     = $cached->avatarMedium;
                $this->avatarFull       = $cached->avatarFull;
                $this->personaState     = $cached->personaState;
            } else {
        ...
        public static function isCached( $st ) {
            if ( array_key_exists( $st, self::$cachedUsers ) ) {
                return self::$cachedUsers[$st];
            }
        }
